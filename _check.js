    ...
    const HISTORY_KEY = "poetry_history_v1";
    ...
    function revealAnswer(msg="揭晓答案", extra={}){
      const q = state.currentQ;
      // Record in history when revealed without choosing
      if (!state.roundHistory.some(h => h.stem === q.stem && h.title === q.title)) {
        state.roundHistory.push({
          title: q.title,
          stem: q.stem,
          answer: q.answer,
    ...
    function bind(){
      ...
      els.historyToggle.addEventListener('click', ()=>{
        const isOpen = !els.historyPanel.hidden;
        els.historyPanel.hidden = isOpen;
        els.historyToggle.textContent = isOpen ? "历史记录" : "收起记录";
    ...
    function renderHistory(){
      ...
      const totalCorrect = list.reduce((s, ses) => s + (ses.correct || 0), 0);
      const totalQuestions = list.reduce((s, ses) => s + (ses.total || 0), 0);
      const overallAcc = totalQuestions ? Math.round((totalCorrect / totalQuestions) * 100) : 0;
      const avgScore = Math.round(list.reduce((s, ses) => s + (ses.score || 0), 0) / totalSessions);
      els.historyStats.innerHTML = `<div>共 ${totalSessions} 次练习</div><div>总正确率 ${overallAcc}% · 平均分 ${avgScore}</div>`;
      els.historyList.innerHTML = list.map(ses => {
        ...
        const acc = ses.total ? Math.round((ses.correct / ses.total) * 100) : 0;
        const lv = (LEVEL_META[ses.level]?.label || ses.level || "初级").replace(/[（].+/, "");
        return `<div class="historyItem"><div class="historyItemHeader"><span>${ds}</span><span class="historyLevel">${lv}</span></div><div class="historyItemStats">得分 ${ses.score} · ${ses.correct}/${ses.total}（${acc}%）</div></div>`;
      }).join('');
    }
